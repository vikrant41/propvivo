...
const SubHeading: React.FC<SubHeadingProps> = ({
  ...
  return (
    <div className={`flex items-center justify-center gap-2.5 ${clsName} ${isLeft && "!justify-start"}`} style={{gap: "10px"}}>
      {showLeftLine && <div className="w-7 h-0.5 bg-lightGreen rounded-xl" style={{height: "3px"}}/>}
      <div className="font-outfit text-base uppercase tracking-wide text-pvBlack">
        {text}
      </div>
      {showRightLine && <div className="w-7 h-0.5 bg-lightGreen rounded-xl" style={{height: "3px"}} />}
    </div>
  );
};
