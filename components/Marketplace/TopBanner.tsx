  ...
  return (
    ...
    >
      <div className="text-center relative z-10 space-y-3">
        <h1 className="text-4xl md:text-6xl text-white font-semibold">{title}</h1>
        <nav className="">
          {breadcrumbs.map((breadcrumb, index) => (
            <span key={index} className="text-white">
              {breadcrumb.href ? (
                <Link href={breadcrumb.href}>
                  <a className="hover:underline text-associationLightgray transition-all duration-300">{breadcrumb.name}</a>
                </Link>
              ) : (
                breadcrumb.name
              )}
